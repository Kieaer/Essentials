...
class Server : Runnable {
    ...
    override fun run() {
        try {
            server = ServerSocket(conf.port)
            while (!Thread.currentThread().isInterrupted) {
                val socket = server!!.accept()
                Log.debug(
                    bundle["network.server.connected", socket.getInetAddress().hostAddress]
                )
                clients.add(socket)
    ...
    fun start(socket: Socket) {
        ...
        Log.info(bundle["network.server.disconnected", socket.getInetAddress().hostAddress])
    }
}