...
class Upgrade {
    fun upgrade() {
        ...
        if (root.child("config.txt").exists()) {
            ...
            edit(config, 21, "server: " + features.get("afkServer").asString())
            ...
            if (root.child("config/config_protect.yaml").exists()) {
                ...
                edit(
                    protect,
                    18,
                    "account: " + if (json.get("plugin").asObject().get("authType")
                            .asString() == "none"
                    ) "false" else "true"
                )
                edit(protect, 21, "authType: " + json.get("plugin").asObject().get("authType").asString())
                edit(protect, 24, "discordURL: " + json.get("discord").asObject().get("discordURL").asString())
                ...
                root.child("config/config_protect.yaml").file().writeText(config.joinToString("\n"))
            } else {
            ...
            if (root.child("config/config_chat.yaml").exists()) {
                ...
                edit(chat, 3, "enabled:" + features.get("chatlimit").asBoolean())
                edit(chat, 5, "language: " + features.get("chatlanguage").asString())
                edit(chat, 9, "enabled: " + features.get("chatBlacklist").asBoolean())
                edit(chat, 11, "regex: " + features.get("chatBlacklistRegex").asString())

                root.child("config/config_chat.yaml").file().writeText(config.joinToString("\n"))
            } else {
            ...
            if (root.child("config/config_bridge.yaml").exists()) {
                ...
                root.child("config/bridge.yaml").file().writeText(config.joinToString("\n"))
            } else {
