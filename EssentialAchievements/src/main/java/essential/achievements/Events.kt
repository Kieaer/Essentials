...
class Events {
        ...
        Events.on(EventType.BlockBuildEndEvent::class.java) { e ->
            if (e.unit.isPlayer) {
                ...
                if (data != null) {
                    ...
                    if (!e.breaking && e.tile.block().name == "water-extractor" && e.tile.floor().isLiquid) {
                    ...
                    if (!e.breaking && (e.tile.block().name == "power-node-large")) {
                    ...
                    if (!e.breaking && e.tile.block().name.contains("turret")) {
                        data.status.put("record.map.clear.noturrets.failed", "1")
                    }

                    // Check for power generators for NoPowerClear achievement
                    if (!e.breaking && (
                        e.tile.block().name.contains("generator") || 
                        e.tile.block().name.contains("solar-panel") ||
                        e.tile.block().name.contains("rtg") ||
                        e.tile.block().name.contains("reactor")
                    )) {
                        data.status.put("record.map.clear.nopower.failed", "1")
                    }

                    // Check for duo turrets for DuoTurretSurvival achievement
                    if (!e.breaking && e.tile.block().name != "duo" && e.tile.block().name.contains("turret")) {
    ...
    fun wave() {
        Events.on(WaveEvent::class.java, { e ->
            players.forEach { data ->
                // Increment action count for APM calculation
                incrementActionCount(data)
    ...
    fun playerChat() {
        Events.on(EventType.PlayerChatEvent::class.java, { e ->
            ...
            } else if (e.message.startsWith("/apm")) {
    ...
    fun unitChange() {
        Events.on(EventType.UnitChangeEvent::class.java, { e ->
            if (e.player != null && e.unit != null) {
                val data: PlayerData? = findPlayerByUuid(e.player.uuid())
                if (data != null) {
                    // Increment action count for APM calculation
                    incrementActionCount(data)

                    if (state.rules.planet === Planets.serpulo && e.unit.type.name.equals("quad", true)) {
                    ...
                    if (!e.unit.type.name.equals("flare", true) && e.unit.type.name != "alpha" && e.unit.type.name != "beta" && e.unit.type.name != "gamma") {
    ...
    fun unitDestroy() {
        Events.on(EventType.UnitDestroyEvent::class.java, { e ->
            ...
            for (player in Groups.player) {
                ...
                if (data != null && e.unit != null && e.unit.team() != player.team()) {
                    // Increment action count for APM calculation
                    incrementActionCount(data)
                    ...
                    if (player.unit().type.name.equals("crawler", true)) {
                        ...
                        if (e.unit.type.name.contains("wall") || e.unit.type.name.contains("turret") || e.unit.type.name.contains("factory")) {
                    ...
                    if (player.unit().type.name.contains("turret") && e.unit.type.name.equals("quill", true)) {
                    ...
                    if (player.unit().type.name.contains("turret") && e.unit.type.name.equals("zenith", true)) {
                        val currentTime = System.currentTimeMillis()
                    ...
                    if (player.unit().type.name.equals("omura", true) && e.unit.type.name.equals("horizon", true)) {
                        val multiKillCount = data.status.getOrDefault("record.omura.horizon.kill.current", "0").toInt() + 1
                    ...
                    if (e.unit.type.name.equals("crawler", true)) {
        ...
        scope.startInfiniteScheduler {
            for (data in players) {
                ...
                if (state.rules.planet === Planets.serpulo) {
                    ...
                    if (state.rules.pvp && data.status.getOrDefault("record.pvp.win.serpulo.tracked", "0").toInt() == 0) {
                    ...
                    if (state.rules.pvp && data.status.getOrDefault("record.pvp.win.erekir.tracked", "0").toInt() == 0) {
    ...
    fun blockDestroy() {
        Events.on(BlockDestroyEvent::class.java) { e ->
            if (e.tile != null) {
                ...
                for (player in Groups.player) {
                    val data: PlayerData? = findPlayerByUuid(player.uuid())
                    if (data != null && player.team() != e.tile.team()) {
                        // Increment action count for APM calculation
                        incrementActionCount(data)
                    }
                }
